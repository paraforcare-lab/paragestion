...
import { supabase, supabaseAdmin } from '../lib/supabase.server.js'
  ...
  try {
    ...
    if (range === '1m') {
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        const dayDepenses = (depenses || []).filter(d => {
          const dDate = new Date(d.date_depense).toISOString().split('T')[0];
    ...
    const { data: recentFactures } = await supabase
      .from('factures')
