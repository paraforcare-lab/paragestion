...
export const BonCommandeDocument = forwardRef<HTMLDivElement, BonCommandeDocumentProps>(
  ({ bon, entreprise }, ref) => {
    ...
    const montantTtc = safeVal(bon, 'montantTtc', 'montant_ttc')
    const montantHt = safeVal(bon, 'montantHt', 'montant_ht')
    const montantTva = safeVal(bon, 'montantTva', 'montant_tva')

    const lignes = bon.lignes || []
    const lineCount = lignes.length
    const compact = lineCount > 10

    const dateCommande = bon.dateCommande
      ? format(new Date(bon.dateCommande), 'dd MMMM yyyy', { locale: fr })
      : ''
    const dateLivraisonPrevue = bon.dateLivraisonPrevue
      ? format(new Date(bon.dateLivraisonPrevue), 'dd MMMM yyyy', { locale: fr })
      : ''
              ...
              <div className="text-right relative min-w-[180px]">
                ...
                <div className="relative pt-7">
                  ...
                  <div className={`mt-1 space-y-0.5 text-slate-500 ${compact ? 'text-[9px]' : 'text-[10px]'}`}>
                    ...
                    {dateLivraisonPrevue && <p>Livraison prévue: {dateLivraisonPrevue}</p>}
                  </div>
                </div>
              </div>
            </div>
