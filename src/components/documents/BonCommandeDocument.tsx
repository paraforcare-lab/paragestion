          ...
          <div className="flex justify-between items-start pt-6">
            ...
            <div className="text-right">
              <div 
                ...
                style={{ background: `linear-gradient(135deg, ${primaryColor}, ${accentColor})` }}
              >
                <h2 className="text-3xl font-black tracking-tight">BON DE COMMANDE</h2>
              </div>
              <div className="mt-4 space-y-1">
                <p className="text-lg font-bold text-gray-800">{bon.numero}</p>
                <p className="text-sm text-gray-500">
                  Date: {format(new Date(bon.dateCommande || bon.date), 'dd MMMM yyyy', { locale: fr })}
                </p>
                {bon.dateLivraisonPrevue && (
                  <p className="text-sm text-gray-500">
                    Livraison prévue: {format(new Date(bon.dateLivraisonPrevue), 'dd MMM yyyy', { locale: fr })}
                  </p>
                )}
              </div>
            </div>
